...
def start_logging(name=None, capture_warnings=True,
                  short_name=False, script_name=True):
    ...
    Args:
      name (string, optional): Name of logger. If no name is supplied
    ...
    # Check if we already have a handler set - e.g. jupyter notebooks
    ...
    for handler in logging.getLogger().handlers:
        handler.setLevel(levels[settings.user_profile["log_level"]])

    # Set log level for "extra" logger
    logging.getLogger("extra").setLevel(
        levels[settings.user_profile["log_level"]])
    ...
    if not name:  # use current module
        current_modules = inspect.getouterframes(inspect.currentframe())[1][1]
        name = current_modules[
            current_modules.rfind("/")+1:current_modules.rfind(".")]

    if logger_set:  # Don't need to create handlers
        # Create logger
        logger = logging.getLogger(name=name)
        logger.setLevel(levels[settings.user_profile["log_level"]])
        return logger
    ...
    # Start logging
    logger = logging.getLogger(name=name)
    logger.info("echidna-v%s" % echidna.__version__)
    logger.info("Saving logfile to %s" % path + filename)
    logger.info("Starting logging: %s" % name)
